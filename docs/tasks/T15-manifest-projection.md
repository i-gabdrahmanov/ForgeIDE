...
- [ ] GWT Т-1/SR-2 автотестом: имитатор дописывает шаг в manifest.json → FAILED(tampered), проекция восстановлена, incident.tamper в аудите
- [ ] контракт-тест: `tdd-guard` из forge-репо корректно резолвит активный шаг по проекции
- [ ] оверхед генерация+сверка ≤ 2 с (NFR-4, часть бюджета)
